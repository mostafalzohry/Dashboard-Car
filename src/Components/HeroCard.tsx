...
interface IHeroCard {
 ...
 const HeroCard: React.FC<IHeroCard> = ({
   ...
   return (
     <div
       className="mx-4 px-[20px] h-[300px] w-[232px] rounded-md flex flex-col items-center justify-center"
       style={{ color: textColor, background: backgroundColor }}
     >
       <img className="w-[38px] h-[38px] mb-4" alt={`${title}`} src={`./${iconPath}`} />
       <h3 className="text-[24px] mb-6">{title}</h3>
       <div className="relative flex justify-center items-center w-[140px] h-[140px]">
         ...
         <span className="absolute text-[32px] font-bold">{`${percent}%`}</span>
       </div>
     </div>
   );
 };
